...
def getMail():
    try:
        clipboard_data = subprocess.check_output(['xclip', '-selection', 'clipboard', '-o']).decode('utf-8')
        if "@dropmail.me" in clipboard_data or "@10mail.org" in clipboard_data or "@emlpro.com" in clipboard_data or "@emltmp.com" in clipboard_data:
...
while True:
    ...
    pyautogui.keyDown('ctrlleft'); pyautogui.typewrite('c'); pyautogui.keyUp('ctrlleft') # ctrlc is interupting the terminal
    # pyautogui.hotkey('ctrlleft', 'c') # didn't work either
    # pyautogui.keyDown('ctrlleft'); pyautogui.typewrite('x'); pyautogui.keyUp('ctrlleft') # changed to ctrlx but still not working
    newMail = getMail()
    if newMail:
